  ...
  return (
    <section className="mt-20 max-w-3xl mx-auto space-y-6">
      <div className="text-center space-y-2">
        <h3 className="text-2xl font-bold text-white font-display">Frequently Asked Questions</h3>
        <p className="text-xs text-slate-400">Everything you need to know about COSMOQ AI Placement Engine.</p>
      </div>
      ...
      <div className="space-y-3">
        {faqs.map((faq, idx) => (
          <div 
            key={idx} 
            className="cosmo-glass-card rounded-2xl border border-white/10 overflow-hidden"
          >
            <button
              onClick={() => { setOpenFaqIndex(openFaqIndex === idx ? null : idx); playAudioFeedback('click'); }}
              className="w-full p-5 text-left flex items-center justify-between text-sm font-bold text-white hover:text-blue-300 transition-colors"
            >
              <span>{faq.q}</span>
              {openFaqIndex === idx ? <ChevronUp className="w-4 h-4 text-blue-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
            </button>
            {openFaqIndex === idx && (
