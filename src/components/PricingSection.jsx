  ...
  return (
    <section className="mt-20 pt-12 border-t border-slate-200">
      <div className="text-center max-w-3xl mx-auto space-y-4 mb-12">
        ...
        <p className="text-xs sm:text-sm text-slate-600">Choose the AI deployment scale for your institution.</p>

        {/* Billing Toggle Switch */}
        <div className="flex items-center justify-center gap-3 pt-2">
          <span className={`text-xs font-bold ${billingCycle === 'monthly' ? 'text-slate-900' : 'text-slate-500'}`}>Monthly</span>
          <button
            onClick={() => { setBillingCycle(billingCycle === 'monthly' ? 'annual' : 'monthly'); playAudioFeedback('click'); }}
            className="w-12 h-6 rounded-full bg-rose-600 p-1 relative transition-colors"
          >
            <div className={`w-4 h-4 rounded-full bg-white transition-transform ${billingCycle === 'annual' ? 'translate-x-6' : 'translate-x-0'}`} />
          </button>
          <span className={`text-xs font-bold flex items-center gap-1.5 ${billingCycle === 'annual' ? 'text-slate-900' : 'text-slate-500'}`}>
            Annual <span className="px-2 py-0.5 rounded-full text-[9px] bg-rose-500/10 text-rose-700 border border-rose-500/20">20% OFF</span>
          </span>
        </div>
      </div>
